...
public class AddFriendGUICallback extends AbstractGUICallback {
    ...
    public AddFriendGUICallback(FriendsListGUIScreen screen, FriendsListGUISection section, int width, int height) {
    ...
    public void init() {      
        ...
        this.addElement(new GUITextLabel(2, 2).setDisplayText(ClientReference.localize("oxygen_friendslist.gui.friends.addFriendCallback"), true, GUISettings.instance().getTitleScale()));   
        this.addElement(new GUITextLabel(2, 16).setDisplayText(ClientReference.localize("oxygen_friendslist.gui.friends.addFriendCallback.request"), false, GUISettings.instance().getTextScale()));  
    ...
    public void handleElementClick(AbstractGUISection section, GUIBaseElement element, int mouseButton) {
        if (element == this.cancelButton)
            ...
            if (FriendsListManagerClient.isPlayerAvailable(this.usernameField.getTypedText())) 
                FriendsListManagerClient.instance().sendFriendRequestSynced(OxygenHelperClient.getSharedPlayerData(this.usernameField.getTypedText()).getPlayerUUID());
